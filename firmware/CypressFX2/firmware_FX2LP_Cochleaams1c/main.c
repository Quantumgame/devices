...
// 
//  tested on and adjusted for CochleaAMS1cV4, Jan 2017, federico corradi  
//
//-----------------------------------------------------------------------------
...
// Device-specific vendor requests new ones
#define VR_MS_FEATURE_DSCR 0xAF
//#define VR_EEPROM 0xa2
#define VR_CPLD_UPLOAD 0xC5 
#define VR_CPLD_CONFIG 0xBF
#define VR_CPLD_CONFIG_MULTIPLE 0xC2
#define VR_CPLD_CONFIG_AER 0xC5
#define VR_CPLD_CONFIG_AER_MULTIPLE 0xC6
...
static BOOL EEPROMRead(WORD addr, BYTE length, BYTE xdata *buf);
static BOOL EEPROMWrite(WORD addr, BYTE length, BYTE xdata *buf);
...
void TD_Init(void) // Called once at startup
{
	...
	IOC = 0x78; // JTAG disabled (TMS, TCK, TDI high), SPI SSN is active-low
	IOE = 0x00; // Keep all off

	SYNCDELAY;
	OEA = 0x00; // 0000_0000, none are used
	OEC = 0x0E; // 0000_1110, JTAG (left floating) and SPI (but not SPI MISO)
	OEE = 0xC0; // 1100_0000, Reset and FXLED only
